...
// var myModal = new bootstrap.Modal(document.getElementById('ModalVendedor'), {
//     keyboard: false
// })
...
function Cargar() {
    if ($.fn.DataTable.isDataTable('#vendedores-table')) {
        $('#vendedores-table').DataTable().destroy();
    }
    let table = $('#vendedores-table').DataTable(
        {
            language: {
            ...
            ajax: '/admin/admin.vendedores.index',
                columns: [
...
function showCustomVend(btn) {
    $.get("/admin/admin.vendedores.edit/" + btn, (response) => {
...
function regVend() {
    ...
    $('#exampleModalLabel').html('Registrar Vendedor');
...
function registerVend() {
    ...
    ajax_data.append('nombre_completo', $('#nombre_completo').val());
...
function updateVend(btn) {
    const route = `/admin/admin.vendedores.update/${btn}`;
