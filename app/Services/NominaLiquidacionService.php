...
class NominaLiquidacionService
{
    ...
    {
        // ── PASO 0: CARGAR PARÁMETROS CONFIGURABLES ──────────────────────
        $pctSaludEmp    = $this->parametros->getFloat('NOM_PCT_SALUD_EMP',       0.04);
        $pctPensionEmp  = $this->parametros->getFloat('NOM_PCT_PENSION_EMP',     0.04);
        $pctSaludEmpr   = $this->parametros->getFloat('NOM_PCT_SALUD_EMPR',      0.085);
        $pctPensionEmpr = $this->parametros->getFloat('NOM_PCT_PENSION_EMPR',    0.12);
        $pctSena        = $this->parametros->getFloat('NOM_PCT_SENA',            0.02);
        $pctIcbf        = $this->parametros->getFloat('NOM_PCT_ICBF',            0.03);
        $pctCaja        = $this->parametros->getFloat('NOM_PCT_CAJA',            0.04);
        $pctPrima       = $this->parametros->getFloat('NOM_PCT_PRIMA',           1 / 12);
        $pctCesantias   = $this->parametros->getFloat('NOM_PCT_CESANTIAS',       1 / 12);
        $pctIntCes      = $this->parametros->getFloat('NOM_PCT_INT_CES',       0.01);
        $pctVacaciones  = $this->parametros->getFloat('NOM_PCT_VACACIONES',      15 / 360);

        $factorRN   = $this->parametros->getFloat('NOM_FACTOR_RN',   1.35);
        $factorHED  = $this->parametros->getFloat('NOM_FACTOR_HED',  1.25);
        $factorHEN  = $this->parametros->getFloat('NOM_FACTOR_HEN',  1.75);
        $factorTDD  = $this->parametros->getFloat('NOM_FACTOR_TDD',  1.75);
        $factorTDN  = $this->parametros->getFloat('NOM_FACTOR_TDN',  2.10);
        $factorHEDD = $this->parametros->getFloat('NOM_FACTOR_HEDD', 2.00);
        $factorHEDN = $this->parametros->getFloat('NOM_FACTOR_HEDN', 2.50);

        $topeAuxSmlv = $this->parametros->getFloat('NOM_TOPE_AUX_SMLV', 2.0);
        $topeIbcSmlv = $this->parametros->getFloat('NOM_TOPE_IBC_SMLV', 25.0);
        ...
        $devRN            = $diasNocturnos * $salDia * $factorRN;
        $devDomDiurno     = $domDiurnos * $salDia * $factorTDD;
        $devDomNocturno   = $domNocturnos * $salDia * $factorTDN;
        $devHED           = $hedHoras * $salHora * $factorHED;
        $devHEN           = $henHoras * $salHora * $factorHEN;
        $devHEDD          = $heddHoras * $salHora * $factorHEDD;
        $devHEDN          = $hednHoras * $salHora * $factorHEDN;
        ...
        $auxTransporte      = ($salBase <= $params->smlv * $topeAuxSmlv) ? (float)$params->aux_transporte : 0.0;
        ...
        $ibc = max(
            min(round($devSalarial), $params->smlv * $topeIbcSmlv),
            (float)$params->smlv
        );
        ...
        $dedSaludEmp   = round($ibc * $pctSaludEmp);
        $dedPensionEmp = round($ibc * $pctPensionEmp);
        $totalDedEmp   = $dedSaludEmp + $dedPensionEmp;

        $netoEmpleado = $totalDevengado - $totalDedEmp;

        // ── PASO 6: SEGURIDAD SOCIAL EMPLEADOR ──────────────────────────
        $costoSaludEmp   = $esExonerado ? 0 : round($ibc * $pctSaludEmpr);
        $costoPensionEmp = round($ibc * $pctPensionEmpr);
        $costoArl        = round($ibc * $arlPorcentaje / 100);
        $subtotalSS      = $costoSaludEmp + $costoPensionEmp + $costoArl;

        // ── PASO 7: PARAFISCALES ────────────────────────────────────────
        $costoSena   = $esExonerado ? 0 : round($ibc * $pctSena);
        $costoIcbf   = $esExonerado ? 0 : round($ibc * $pctIcbf);
        $costoCaja   = round($ibc * $pctCaja);
        ...
        $provPrima        = round($basePrestaciones * $pctPrima);
        $provCesantias    = round($basePrestaciones * $pctCesantias);
        $provIntCesantias = round($provCesantias * $pctIntCes);
        $provVacaciones   = round($baseVacaciones * $pctVacaciones);
    ...
    /**
     * Retorna el porcentaje ARL según el nivel desde la BD (nom_arl_niveles).
     * Fallback a los valores legales vigentes si el nivel no existe.
     */
    private function getPorcentajeArl(int $nivel): float
    {
        $registro = NominaArlNivel::where('nivel', $nivel)->first();
