        ...
        ): int
    {
        ...
        return DB::transaction(function () use (
            ...
            foreach ($settlements as $s) {
                ...
                // 9) Outbox para n8n (idempotente por settlement)
                $eventKey = 'production.settlement.synced:' . (int) $s->id;

                IntegrationOutboxEvent::updateOrCreate(
                    [
                        'event_key' => $eventKey,
                    ],
                    [
                        'company_id' => $companyId,
                        'event_type' => 'production.settlement.synced',
                        'payload' => [
                            'event_type' => 'production.settlement.synced',
                            'company_id' => (int) $companyId,
                            'order_id' => (int) $order->id,
                            'pay_run_id' => (int) $payRun->id,
                            'settlement_id' => (int) $s->id,
                            'employee_id' => $employeeId,
                            'order_operation_id' => (int) ($s->order_operation_id ?? 0),
                            'source_ref' => $sourceRef,
                            'period_start' => (string) $payRun->period_start,
                            'period_end' => (string) $payRun->period_end,
                            'qty' => (float) $qty,
                            'rate' => (float) ($s->rate ?? 0),
                            'amount' => round($amount, 2),
